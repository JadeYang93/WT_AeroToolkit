...
> 模块 ID：`shape_output`　　默认输入：`输入数据\blade_shape\`　　默认输出：`输出\blade_shape\`
...
└── AppData\
    ...
    └── Blade_database\
        └── <你的叶片>.geo       # 7 列：展长 弦长 扭角 相对厚度 变桨轴 预弯 后掠
```
...
| 字段 | 说明 |
|---|---|
| AppData 输入目录 | 上面的 AppData 根目录（默认 `输入数据\blade_shape\AppData\`） |
| 🔍 扫描 | 扫描 `AppData\Blade_database\` 刷新 GEO 下拉；扫描 `AppData\Aerofoil_coordinate\` 刷新翼型库下拉 |
| GEO 输入文件 | 下拉选择 `.geo` 文件；点 `...` 可浏览外部文件；点 `🔄` 只刷新本下拉 |
| 翼型库（可空） | 下拉选择翼型族（`Aerofoil_coordinate\` 下的子目录）；留空 → 用 `configuration.txt` 指定的默认族；点 `...` 可浏览外部文件夹；点 `🔄` 只刷新本下拉 |
| STAGE-1 输出目录 | 默认 `输出\blade_shape\stage1\` |
...
顶部展示**修正示意图**（`src\_assets\shape_output\te_correction_diagram.png`），
下方是规则说明文字与参数表。
...
**目标文件**：默认 `输出\blade_shape\stage2\GEO_for_correction.xlsx`。
...
| 字段 | 说明 |
|---|---|
| XFOIL 工作目录 | 默认 `输出\blade_shape\_internal\xfoil_work\` |
| 最终输出目录 | 默认 `输出\blade_shape\stage3\` |
...
### 4.1 STAGE-1（`输出\blade_shape\stage1\`）
...
### 4.2 STAGE-2（`输出\blade_shape\stage2\`）
...
### 4.3 STAGE-3（`输出\blade_shape\stage3\`）
...
```
[STAGE-1 Tab]
  AppData + GEO → build_shape_output → 6 个 stage1/*.xlsx（含 GEO 模板）
                                        ↓
...
| STAGE-1 日志「.prof files found in」 | AppData 的 `Aerofoil_coordinate\<族名>\` 下没有 `.prof` 文件 |
...
| GEO 下拉框为空 | 点 GEO 旁 **🔄** 单独刷新；若仍空，确认 `AppData\Blade_database\` 下有 `.geo` 文件 |
| 翼型库下拉框只有「默认」 | 点翼型库旁 **🔄** 单独刷新；确认 `AppData\Aerofoil_coordinate\<族名>\` 下有 `.prof` 文件，子目录无 `.prof` 不会出现在下拉里 |
