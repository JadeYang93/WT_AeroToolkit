...
输入/输出路径走 ConfigCenter（默认 输入数据/shape_output/、输出/shape_output/）。
"""
import os
import re
import sys
...
class Stage1Worker(QThread):
    """STAGE-1：AppData + GEO → build_shape_output → export + prepare correction."""
    ...
    def run(self):
        try:
            self._log(5, '加载输入数据...')
            if self.profile_dir:
                base = load_shape_output_input(self.appdata, self.geo_path, profile_family=None)
                data = type(base)(
                    geo=base.geo,
                ...
                data = load_shape_output_input(self.appdata, self.geo_path, profile_family=None)
            self._log(30, '计算外形输出...')
            options = ShapeOutputOptions(
            ...
            result = build_shape_output(data, options)
            self._log(60, '导出文件...')
            written = export_shape_output(result, self.stage1_output, options, layout='flat')
            self._log(80, '准备修正输入...')
            # STAGE-1 产物直接平铺在 stage1/ 下（layout='flat'）。
            # TEth_baseline.npy 写到 _internal/ 隐藏工作区，避免污染用户可见目录。
            correction_input_dir = Path(self.stage1_output)
            baseline_dir = Path(self.stage1_output).parent / '_internal'
            baseline_dir.mkdir(parents=True, exist_ok=True)
            correction_inputs = prepare_correction_inputs(
                self.stage1_output, correction_input_dir, baseline_dir=baseline_dir,
            )
...
class Stage2Worker(QThread):
    ...
    def run(self):
        try:
            ...
            options = ShapeOutputOptions(**self.export_flags)

            # ----- 阶段 C：导出 (80% → 100%) -----
            written = export_shape_output(
    ...
    def _make_export_cb(self):
        """export_shape_output 回调（80% → 100%）。按已见文件数线性插值。"""
        state = {'count': 0}
        def cb(msg):
            state['count'] += 1
...
class ShapeOutputPanel(QWidget):
    MODULE_ID = 'shape_output'
    DEFAULT_INPUT_SUBDIR = 'blade_shape'
    DEFAULT_OUTPUT_SUBDIR = 'blade_shape'
    ...
    def _sync_path_edits(self):
        ...
        for edit, key in [
            (getattr(self, 'appdata_edit', None), 'appdata'),
    ...
    def _default_paths(self):
        appdata = Path(self.input_dir) / 'AppData'
        return {
            'appdata': str(appdata),
            'blade_database': str(appdata / 'Blade_database'),
    ...
    def _build_stage1_tab(self, defaults):
        ...
        grid.setSpacing(6)
        # row0: AppData 根目录 + 扫描按钮
        grid.addWidget(QLabel('AppData 输入目录:'), 0, 0)
        self.appdata_edit = QLineEdit(defaults['appdata'])
        grid.addWidget(self.appdata_edit, 0, 1)
        cell = QHBoxLayout()
        cell.setSpacing(4)
        cell.addWidget(self._browse_btn(self.appdata_edit, 'dir'))
        scan_btn = QPushButton('🔍 扫描')
        scan_btn.setMaximumWidth(80)
        scan_btn.setToolTip('扫描 Aerofoil_coordinate/ 和 Blade_database/，刷新下面两个下拉框')
        scan_btn.clicked.connect(self._on_scan_appdata)
        cell.addWidget(scan_btn)
        cell_w = QWidget()
        cell_w.setLayout(cell)
        grid.addWidget(cell_w, 0, 2)
        # row1: GEO 文件下拉 + 浏览 + 刷新
        grid.addWidget(QLabel('GEO 输入文件:'), 1, 0)
        self.geo_combo = QComboBox()
        self.geo_combo.setToolTip('选择 .geo 文件（来自 Blade_database/ + Temp_GEO/）')
        grid.addWidget(self.geo_combo, 1, 1)
        geo_cell = QHBoxLayout()
        geo_cell.setSpacing(4)
        geo_browse = QPushButton('...')
        geo_browse.setMaximumWidth(40)
        geo_browse.setToolTip('浏览其他 .geo 文件')
        geo_browse.clicked.connect(self._on_browse_geo)
        geo_refresh = QPushButton('🔄')
        geo_refresh.setMaximumWidth(40)
        geo_refresh.setToolTip('重新扫描 Blade_database/ 和 Temp_GEO/，刷新本下拉框')
        geo_refresh.clicked.connect(self._on_refresh_geo)
        geo_cell.addWidget(geo_browse)
        geo_cell.addWidget(geo_refresh)
        geo_cell_w = QWidget()
        geo_cell_w.setLayout(geo_cell)
        grid.addWidget(geo_cell_w, 1, 2)
        # row2: 翼型库下拉（空选项 = 用 configuration.txt 指定的默认族）+ 浏览 + 刷新
        grid.addWidget(QLabel('翼型库:'), 2, 0)
        self.profile_combo = QComboBox()
        self.profile_combo.setToolTip('选择翼型族（Aerofoil_coordinate/ 下的子目录）')
        grid.addWidget(self.profile_combo, 2, 1)
        prof_cell = QHBoxLayout()
        prof_cell.setSpacing(4)
        prof_browse = QPushButton('...')
        prof_browse.setMaximumWidth(40)
        prof_browse.setToolTip('浏览其他翼型库文件夹')
        prof_browse.clicked.connect(self._on_browse_profile)
        prof_refresh = QPushButton('🔄')
        prof_refresh.setMaximumWidth(40)
        prof_refresh.setToolTip('重新扫描 Aerofoil_coordinate/，刷新本下拉框')
        prof_refresh.clicked.connect(self._on_refresh_profile)
        prof_cell.addWidget(prof_browse)
        prof_cell.addWidget(prof_refresh)
        prof_cell_w = QWidget()
        prof_cell_w.setLayout(prof_cell)
        grid.addWidget(prof_cell_w, 2, 2)
        # row3: STAGE-1 输出
        grid.addWidget(QLabel('STAGE-1 输出目录:'), 3, 0)
        self.stage1_output_edit = QLineEdit(defaults['stage1_output'])
        grid.addWidget(self.stage1_output_edit, 3, 1)
        grid.addWidget(self._browse_btn(self.stage1_output_edit, 'dir'), 3, 2)
        ...
        # 说明
        info = QGroupBox('本阶段固定输出（STAGE-3 修正输入的来源）')
        ...
        info_label = QLabel(
            ...
            '  standard_airfoil_for_correction.xlsx    翼型点云输入'
        )
    ...
    def _build_te_tab(self, defaults):
        ...
        # 说明（示意图 + 文字）
        desc = QGroupBox('说明')
        desc.setObjectName('gb_desc')
        desc_layout = QVBoxLayout(desc)
        desc_layout.setSpacing(8)

        # 示意图
        diagram_path = SRC_DIR / '_assets' / 'shape_output' / 'te_correction_diagram.png'
        if diagram_path.exists():
            self.te_diagram_label = QLabel()
            pix = QPixmap(str(diagram_path))
            if not pix.isNull():
                # 等比缩放到合适宽度（保留高清）
                max_w = 720
                if pix.width() > max_w:
                ...
                self.te_diagram_label.setPixmap(pix)
                self.te_diagram_label.setAlignment(Qt.AlignCenter)
                self.te_diagram_label.setMinimumHeight(
                    max(160, int(pix.height() * 0.6)))
                desc_layout.addWidget(self.te_diagram_label,
                                      alignment=Qt.AlignCenter)

        # 文字说明
        desc_label = QLabel(
            '按 span/R 无量纲位置批量改写 GEO_for_correction.xlsx 的 TEth 列。\n'
            '  • t < p4：保留原始值\n'
            '  • p4 ≤ t < p1：余弦光顺过渡，原始值 → p2\n'
            '  • p1 ≤ t < 最后截面：保持 p2\n'
            '  • 最后截面（叶尖）：p3'
        )
        ...
        # 目标文件（STAGE-2 改写对象：首次运行时从 stage1/ 复制到 stage2/）
    ...
    def _build_stage2_tab(self, defaults):
        ...
        grid.setContentsMargins(10, 8, 10, 8)
        grid.setSpacing(6)
        grid.addWidget(QLabel('XFOIL 工作目录:'), 0, 0)
        self.correction_work_edit = QLineEdit(defaults['correction_work'])
        grid.addWidget(self.correction_work_edit, 0, 1)
        grid.addWidget(self._browse_btn(self.correction_work_edit, 'dir'), 0, 2)
        grid.addWidget(QLabel('最终输出目录:'), 1, 0)
        self.final_output_edit = QLineEdit(defaults['final_output'])
        grid.addWidget(self.final_output_edit, 1, 1)
        grid.addWidget(self._browse_btn(self.final_output_edit, 'dir'), 1, 2)
        grid.setColumnStretch(1, 1)
        v.addWidget(box)
        ...
        self.cb_export_airfoil = QCheckBox('标准翼型点云  standard_airfoil_points.xlsx')
        self.cb_export_airfoil.setChecked(True)
        self.cb_export_3d = QCheckBox('3D 叶片点云  blade_3d_points.xlsx')
        self.cb_export_3d.setChecked(True)
        self.cb_export_geom = QCheckBox('几何参数表  blade_aero_geometry.xlsx')
        self.cb_export_geom.setChecked(True)
        self.cb_export_tail = QCheckBox('修正后尾缘厚度  trailing_edge_thickness.xlsx')
        self.cb_export_tail.setChecked(True)
        self.cb_export_focus = QCheckBox('Focus.mac')
        self.cb_export_focus.setChecked(True)
        self.cb_export_stp = QCheckBox('STP 点云  3D_points.stp')
        self.cb_export_stp.setChecked(True)
        for cb in (self.cb_export_airfoil, self.cb_export_3d, self.cb_export_geom,
                   self.cb_export_tail, self.cb_export_focus, self.cb_export_stp):
            out_layout.addWidget(cb)
        out_layout.addStretch()

        # XFOIL 模式
        mode_box = QGroupBox('XFOIL 模式')
        mode_layout = QVBoxLayout(mode_box)
        self.xfoil_mode_group = QButtonGroup(self)
        self.rb_mode_run = QRadioButton('完整运行（修正 + 重建）')
        self.rb_mode_run.setChecked(True)
        self.rb_mode_skip = QRadioButton('跳过 XFOIL（用现有翼型直接重建）')
        self.rb_mode_debug = QRadioButton('调试模式（完整运行 + 保留临时文件）')
        self.xfoil_mode_group.addButton(self.rb_mode_run, 0)
        self.xfoil_mode_group.addButton(self.rb_mode_skip, 1)
        self.xfoil_mode_group.addButton(self.rb_mode_debug, 2)
        mode_layout.addWidget(self.rb_mode_run)
        mode_layout.addWidget(self.rb_mode_skip)
        mode_layout.addWidget(self.rb_mode_debug)
        mode_layout.addStretch()

        opts_layout.addWidget(out_box, 1)
    ...
    def _on_scan_appdata(self, silent=False):
        """扫描 AppData/ 下的 Blade_database + Aerofoil_coordinate，
        刷新 GEO 文件下拉与翼型库下拉。

        silent=True 时不写日志（首次构建 / 路径变更联动用）。
        """
        appdata = self.appdata_edit.text().strip()
        if not appdata or not Path(appdata).exists():
            if not silent:
                self._log('stage1', f'[扫描] ⚠ AppData 目录不存在：{appdata}')
            return 0, 0

        n_geo = self._scan_geo_combo(appdata)
        n_prof = self._scan_profile_combo(appdata)

        if not silent:
            self._log('stage1', f'[扫描] {appdata}')
    ...
    def _on_refresh_geo(self):
        ...
        if not appdata or not Path(appdata).exists():
            QMessageBox.warning(self, '路径无效',
                                f'AppData 目录不存在：\n{appdata}')
            return
        n = self._scan_geo_combo(appdata)
    ...
    def _scan_geo_combo(self, appdata):
        """扫描 Blade_database/，刷新 GEO 下拉。
        返回扫描到的 GEO 文件数。保留用户浏览过的外部项。
        """
        geo_files = []
        blade_db = Path(appdata) / 'Blade_database'
        if blade_db.is_dir():
            for p in sorted(blade_db.glob('*.geo')):
    ...
    def _on_run_stage1(self):
        ...
        appdata = self.appdata_edit.text().strip()
        if not appdata:
            QMessageBox.warning(self, '缺少 AppData', '请先选择 AppData 输入目录。')
            return
        profile_dir = self.profile_combo.currentData() or ''
        if not profile_dir:
            QMessageBox.warning(self, '缺少翼型库',
                                '请先选择翼型库（Aerofoil_coordinate/ 下的翼型族）。\n'
                                '若列表为空，点 AppData 旁的 🔍 扫描 或翼型库旁的 🔄 刷新。')
            return
        # 清空日志 + 进度
        ...
        self._log('stage1', f'输入: AppData={appdata}')
    ...
    def _on_run_stage2(self):
        ...
        correction_airfoil = stage1_out / 'standard_airfoil_for_correction.xlsx'
        ...
        export_flags = {
            'export_airfoil_points': self.cb_export_airfoil.isChecked(),
            'export_3d_points': self.cb_export_3d.isChecked(),
            'export_geometry': self.cb_export_geom.isChecked(),
            'export_tail': self.cb_export_tail.isChecked(),
            'export_focus': self.cb_export_focus.isChecked(),
            'export_step_points': self.cb_export_stp.isChecked(),
        }
        ...
        self._stage2_worker = Stage2Worker(
            ...
            final_output, mode, self.xfoil_exe, export_flags,
        )
