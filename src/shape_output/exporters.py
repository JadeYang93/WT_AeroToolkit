"""Exporters for standalone shape output data."""
...
from .models import ShapeOutputOptions, ShapeOutputResult
...
def _section_matrix_header(result: ShapeOutputResult, labels: tuple[str, ...]) -> list[str]:
...
def write_focus_file(path: str | Path, result: ShapeOutputResult) -> Path:
...
def write_step_points_file(path: str | Path, result: ShapeOutputResult) -> Path:
    ...
    with path.open("w", encoding="utf-8", newline="") as f:
        ...
        f.write(f"FILE_NAME('','{date_text}',('none'),('none'),'Python','shape_output','none');\n")
        f.write("FILE_SCHEMA(('CONFIG_CONTROL_DESIGN'));\n")
        f.write("ENDSEC;\n")
        f.write("/* file written by Python shape_output */\n")
        f.write("DATA;\n")
...
def export_shape_output(
    result: ShapeOutputResult,
    output_dir: str | Path,
    options: ShapeOutputOptions | None = None,
    progress_callback=None,
    layout: str = "split",
) -> dict[str, Path]:
    """Write selected shape output files and return their paths.
    ...
    options = options or ShapeOutputOptions()
