...
并行执行FOCUS6计算步骤，提升整体效率：
阶段1: 并行执行 [读取mac文件] 和 [载荷转化]
阶段2: 串行执行 [解析mac文件]
阶段3: 并行执行 [重量] [频率] [应变] [叶尖挠度]
"""
...
class OneClickRunThread(QThread):
    ...
    def run(self):
        ...
        try:
            ...
            self.log_signal.emit(f"开始一键运行 - 并行优化版本")
            ...
            self.log_signal.emit(f"阶段1: 并行执行 [读取mac文件] 和 [载荷转化]")
            self.log_signal.emit(f"{'='*80}")

            has_strain_calculation = zspan_file and zspan_file.exists() and load_file and load_file.exists()

            with ThreadPoolExecutor(max_workers=2) as executor:
                # 提交任务1: 读取mac文件
                future_step1 = executor.submit(
                    self._run_step, 1, "读取mac文件", FUNCTION_READ_MAC,
                    {
                ...
                if has_strain_calculation:
                    future_step5a = executor.submit(
                        self._run_convert_loadcase_only,
                        optimization_folder, solver_type, modules_path,
                        str(load_file), zspan_file, radius
                    )

                # 并行等待所有任务完成
                futures = {
                    '读取mac文件': future_step1,
                }
                if future_step5a:
                    futures['载荷转化'] = future_step5a
                ...
                # 检查任务1的结果
                step1_success = results.get('读取mac文件', True)
                if not step1_success:
                    self._finish(False, "读取mac文件失败")
                    return

                # 检查任务5a的结果（如果存在）
                step5a_success = True
                if '载荷转化' in results:
                    step5a_success = results['载荷转化']
                    if not step5a_success:
                        self.log_signal.emit(f"\n⚠ 警告: 载荷转化失败，将跳过应变计算和叶尖挠度计算")
                        has_strain_calculation = False

            # ========== 阶段2: 串行执行步骤2 ==========
            self.log_signal.emit(f"\n{'='*80}")
            self.log_signal.emit(f"阶段2: 串行执行 [解析mac文件]")
            self.log_signal.emit(f"{'='*80}")
            ...
            if not self._run_step(2, "解析mac文件", FUNCTION_PARSE_MAC, {
                ...
                self._finish(False, "解析mac文件失败")
                return

            # ========== 阶段3: 并行执行步骤3、4、5b、6 ==========
            self.log_signal.emit(f"\n{'='*80}")
            self.log_signal.emit(f"阶段3: 并行执行 [重量] [频率] [应变] [叶尖挠度]")
            self.log_signal.emit(f"{'='*80}")
            ...
            with ThreadPoolExecutor(max_workers=4) as executor:
                ...
                # 提交任务3: 计算重量
                futures['步骤3-重量'] = executor.submit(
                    self._run_step, 3, "计算重量", FUNCTION_WEIGHT,
                    {
                        'solver_type': solver_type,
                ...
                # 提交任务4: 计算频率
                futures['步骤4-频率'] = executor.submit(
                    self._run_step, 4, "计算频率", FUNCTION_FREQUENCY,
                    {
                        'solver_type': solver_type,
                        'mac_solver_type': solver_type,
                ...
                # 提交任务5b: 应变计算（如果需要，跳过载荷转化）
                if has_strain_calculation:
                    futures['步骤5b-应变'] = executor.submit(
                        self._run_step, 5, "计算应变", FUNCTION_STRAIN,
                        {
                            'solver_type': solver_type,
                        ...
                        skip_loadcase_conversion=True  # 跳过载荷转化，已在阶段1完成
                    )

                # 提交任务6: 叶尖挠度计算（仅frbex）
                if solver_type == SOLVER_FRBEX and step5a_success:
                    futures['步骤6-叶尖挠度'] = executor.submit(
                        self._run_step, 6, "计算叶尖挠度", FUNCTION_TIP_DEFLECTION,
                        {
                            'solver_type': solver_type,
                            'mac_solver_type': solver_type,
    ...
    def _run_step(self, step_num, step_name, function_name, params, skip_loadcase_conversion=False):
        ...
        Args:
            ...
            function_name: 功能名称
            params: 参数字典
            skip_loadcase_conversion: 是否跳过载荷转化（用于阶段3的应变计算）
